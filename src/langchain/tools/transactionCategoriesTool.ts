...
export function makeTransactionCategoriesTool(firestore: Firestore) {
  return new DynamicStructuredTool({
    name: 'list_transaction_categories',
    description:
      "Get all documents from 'transactionCategories' with {id,name}. If 'query' is provided, returns fuzzy matches to help validate user typos.",
    schema: z.object({
      query: z.string().optional(),
      topK: z.number().int().positive().max(50).default(5)
    }),
    async func(args) {
      const snap = await firestore.collection('transactionCategories').get();
